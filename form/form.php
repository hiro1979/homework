...
if($pref ==8 or $pref ==10 or $pref ==11 or $pref ==12){
  $noplace ="観光スポットはありません";
}
...
$places = array(
  ...
  14 => array(
    ...
    array(
      'name'   => '旅館の部屋',      'detail' => '旅館の部屋はとても良い所です。旅館の部屋はとても良い所です。
...
// if($pref==9)
//   {echo $places[9]['name'].'<br>'.$places[9]['detail'].'<br>'."<img src=".$places[9]['image'].">";}
// exit;
?>
<!DOCTYPE html>
<html>
  ...
  <div class="container">
    <form class="form-inline" action="" method="post">
      <div class="form-group">
        <select name="pref" class="form-control">
          ...
          foreach($prefectures as $n => $prefecture ){
            echo "<option value='$n'>".$prefecture.'</option>';
          }
          ?>
        </select>
      </div>
     ...
     <?php
      ...
      if($_POST['pref']=='')
      {echo'  <hr> <footer>&copy; 観光スポット検索協会 </footer>';
       exit;}



    if($pref==8 or $pref ==10 or $pref ==11 or $pref ==12 ){
      echo $noplace.'  <hr> <footer>&copy; 観光スポット検索協会 </footer>';
    exit;}
    ?>


    <p class="search-result"><?php echo $prefectures[$pref].'の観光スポットは'; ?>
                            <?php echo count($places[$pref]);?>
                            <?php echo '件見つかりました。';?> </p>
      ...
      <?php foreach($places[$pref] as $place):?>
        ...
        <?php echo '<div class="media-left">'; ?>
        <img src="<?php echo $place['image'] ;?>" class="media-object img-thumbnail">
        <?php echo ' </div>'.'</div>' ;?>
        <?php echo '<div class="media-body">'.'<h4 class="media-heading">'.$place['name'].'</h4>'.$place['detail'].'</div>';?>
      <?php endforeach ?>
